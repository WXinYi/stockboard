...
2. **样式丑** — 与设计稿(`full-page-real.html` 的三区白底图表、±10% 等分 Y 轴、底部时间轴)不匹配。
...
| 4 | 分时视图自绘 | 分时线+均价线+昨收/涨跌停参考线+±10% Y 轴+底部时间轴 |
...
### 6.6 `priceTicks(min, max, rect)` → `[{y, label}]`

主图右侧 Y 轴刻度。

- **分时视图**: **Y 轴固定涨停板边界** —— 上界 = 涨停价 `quote.upPx`,下界 = 跌停价 `quote.downPx`,昨收居中。5 等分(每格标签显示 **百分比 + 对应价格**,如 `+5.0% 94.54`,见设计稿 `.cd-ax`)。百分比按 `(price - prevClose) / prevClose` 计算,支持任意涨跌停幅度(主板 ±10%、创业板/科创板 ±20%、ST ±5%、北交所 ±30%)—— 不硬编码 ±10%,跟随 `quote.upPx/downPx`(由 `calcLimitPx` 按板块算出)。
- **K 线视图**: 按窗口实际 min/max 5 等分,仅显示价格。
...
| `priceTicks` | 分时 Y 轴 = 涨停 `upPx` 上界 / 跌停 `downPx` 下界 / 昨收居中,5 等分百分比+价格;K 线按实际范围 |
...
4. **Y 轴固定涨停板**: 上界 = 涨停价 `quote.upPx`、下界 = 跌停价 `quote.downPx`、昨收居中。5 等分,每格 `+x.x% 价格` 双值(设计稿 `.cd-ax`);百分比按板块涨跌停幅度算,非固定 ±10%。无 `upPx/downPx` 数据时回退 `prevClose×(1±10%)`。
...
- **手工验证清单**(用户在浏览器确认):
  - 分时视图: 分时线红、均价黄、昨收虚线、Y 轴固定涨停板(涨/跌停边界,创业板 ±20% 正确)、X 轴固定全天(09:30~15:00,只画到当前时刻)、5 时间刻度。
