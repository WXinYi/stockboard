...
| `quote` | Object | ✓ | `{prevClose, upPx, downPx, price, name, code}` 等;驱动分时 Y 轴/参考线 |
...
底部时间轴刻度。
- **分时** (`isIntraday && view==='trend'`): 固定 5 个刻度 `09:30 | 10:30 | 11:30/13:00 | 14:00 | 15:00`,等分宽;`11:30/13:00` 为午休合并标签。
...
主图右侧 Y 轴刻度。分时视图固定 `[+10%, +5%, 0%, -5%, -10%]` 5 等分(每格标签显示 **百分比 + 对应价格**,如 `+5.0% 94.54`,见设计稿 `.cd-ax`);K 线视图按实际 min/max 5 等分,仅显示价格。
...
| `timeTicks` | 分时 5 固定刻度;K 线含首末;间隔均匀 |
| `priceTicks` | 分时 5 等分含 +10/-10;K 线按实际范围 |
...
3. **涨/跌停线**: 顶部/底部虚线 `rgba(231,76,60,.55)` / `rgba(39,174,96,.55)`(位置由 `quote.upPx/downPx` 相对昨收的百分比决定;无涨跌停数据则忽略)。
4. **Y 轴标签**: 右侧 5 等分,每格 `+x.x% 价格` 双值(设计稿 `.cd-ax`)。
...
- **手工验证清单**(用户在浏览器确认):
  - 分时视图: 分时线红、均价黄、昨收虚线、±10% 轴、5 时间刻度。
