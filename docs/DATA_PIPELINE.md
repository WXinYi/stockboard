...
- **一次性回补 workflow** `.github/workflows/backfill-pool-history.yml`：push 自身或脚本即触发；**先确认没有进行中的 crawl 班次**再恢复热层→回补→上传（否则 crawl 的 EOD 上传会用无回补的库覆盖，09-09 实测踩坑）。2026-09-15 改两处：① 等待方式由固定 `sleep 20min` 改为**轮询 crawl 是否有 in_progress 班次**（上限 40min，超时 `::warning::` 但继续）——盲等遇到超长班次仍会竞态；② 幂等守卫由"全局 MIN ≤ 2025-09-01 即跳过"改为**按请求区间起点判断**——原先指定更早区间手动触发会被静默忽略（报成功但什么都没做）。
