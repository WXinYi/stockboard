...
<template>
  <div class="stats-row">
    ...
    <div class="stat-card" :class="{ clickable: stockStats.length }" @click="stockStats.length && router.push('/stocks')"><div class="stat-val">{{ stockStats.reduce((s,x)=>s+x.h,0) }}</div><div class="stat-lbl">持仓记录</div></div>
    <div class="stat-card clickable" @click="router.push('/trades')"><div class="stat-val">{{ tradeConsensus.reduce((s,x)=>s+x.bc+x.sc,0) }}</div><div class="stat-lbl">调仓笔数</div></div>
